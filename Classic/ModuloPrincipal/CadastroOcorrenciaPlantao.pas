...
// Cadastra todas as ocorrências dos Adjuntos no plantão.
interface
...
{  Chave1:= IntToStr(GLOBAL_ID_UP);
  ...
  codigo := DBGenerator('ID_OCORRENCIA_PLANTAO');
  DSCadastro.DataSet.FieldByName('ID_OCORRENCIA_PLANTAO').AsInteger := codigo;
...
begin
  ...
  DsNaturezaOcorrencia.DataSet.Close;  // Todos os dataset do formulários devem ser fechados e abertos no FormCreate
  ...
  DsHistOcoInt.DataSet.Close;  // Todos os dataset do formulários devem ser fechados e abertos no FormCreate
  DsHistOcoInt.DataSet.Open;


  // Abrir somente as ocorrências da unidade prisional logada pelo usuário
...
// Seleciona o interno pelo prontuário
  ...
  begin
    ...
    {A pesquisa agora deve retornar todos os internos devido a possibilidade de transferência do mesmo}
    DsInterno.DataSet.Open;
...
begin
  ...
  inherited;
{ Esse procedimento é para não deixar lixo no DataSet do Histórico - Sempre necessário. }
  DSHistOcoInt.DataSet.Close;
  DSHistOcoInt.DataSet.Open;
{ Usar LOCATE para voltar para o mesmo registro depois de salvo, com [loCaseInsensitive, loPartialKey] verifica minúscula
e maiúscula e string parcial}
  ...
  if (EditLocalizar.Text = '') then
   ...
   if RadioGroupTipoLocalizar.ItemIndex = 1 then
     ...
     DsConsulta.dataset.open;
       ...
       Showmessage('Ocorrência não localizada!')
   end;
end;
...
begin
  inherited;
// Chama o relatório para impressão diretamente do Formulário.
...
procedure TFrmCadastroOcorrenciaPlantao.CdsCadastroAfterInsert(
  DataSet: TDataSet);
begin
  inherited;
  //DataSet.FieldByName('ID_OCORRENCIA_PLANTAO').NewValue := DBGENERATOR('ID_OCORRENCIA_PLANTAO');
end;

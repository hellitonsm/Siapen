...
uses
  ...
  Dialogs, Menus, jpeg, ExtCtrls, Buttons, StdCtrls, WinSkinStore,
  WinSkinData, Grids, frxClass, frxPreview, frxChart, ComCtrls, ImgList,
...
type
  TFrmModuloPrincipal = class(TForm)
    ...
    procedure CdsEscalaPlantaoPostError(DataSet: TDataSet;
...
uses CadastroCela, CadastroInternos, CadastroGaleria, CadastroPavilhao,
  ...
  CadastroPsiquiatria, CadastroOdontologia, Cadastropsicologia, CadastroInternoJuridico,
...
begin
  ...
  begin
    ...
    if Sender is TBitBtn then
      ShowMessage('Não Há Permissão Para Acesso - ' + TBitBtn(Sender).Caption);
...
begin
  FrmCadastrofuncaofuncionario := TFrmCadastroFuncaoFuncionario.Create(Application);
...
procedure TFrmModuloPrincipal.MudarComEnter(var Msg: TMsg; var Handled: Boolean);
begin
...
begin
  ...
  PanelCabecalho.Caption := 'SIAPEN - Sistema Integrado de Administração Penitenciária';
  {
    while fazendo do
    begin
...
begin
  ...
  LabelLOGIN.Caption := 'USUÁRIO: ' + LOGIN_CONECTADO + ' | ' + NOME_PERFILUSUARIO_LOGADO;
...
begin
  ...
  FrmCadastroInternoPsicossocial := TFrmCadastroInternoPsicossocial.Create(Application);
...
begin
  ...
  ShowMessage('Os lançamentos de históricos deverão ser feitos através do respectivo módulo do Setor!'
    + #13#13 + 'Recomenda-se não usar o mesmo!');
...
begin
  ...
  begin
    ...
    try
      DM.SQLConnect.ExecuteDirect('update conexao set tela_momento = ' + qs('Modulo Principal')
...
//      begin
//        // Copiar o arquivo da pasta do aplicativo para a pasta do usuário
...
var
  rel1, rel2,DB_LIBRARYNAME: string;
begin
 ...
 { frxReport1.Preview := nil;
  ...
  begin
    ...
    //frxReport1.Variables.AddVariable('SIAP', 'DB_LIBRARYNAM', DB_LIBRARYNAME);
  end;
...
//  frxReport2.EngineOptions.DestroyForms := False;
  ...
  begin
    ...
    frxReport2.Variables.AddVariable('SIAP', 'ID_FUNCIONARIO', GLOBAL_ID_FUNCIONARIO);
    //frxReport2.Variables.AddVariable('SIAP', 'DB_LIBRARYNAM', DB_LIBRARYNAME);
    //frxReport2.Variables.Variables['DB_LIBRARYNAM'] := DB_LIBRARYNAME;
  end;
  try
    if FileExists(rel1) then
      frxReport1.PrepareReport;
...
begin
  if (PERMISSAO_TERAPIAOCUPACIONAL = '') or (PERMISSAO_TERAPIAOCUPACIONAL = 'R') then
  begin
...
begin
  ...
  begin
    ...
    FrmMovimentoProcedimentos.IDPROCEDIMENTOS := FrmFiltroProcedimentos.DBLookupComboBoxProcedimentos.KeyValue;
    FrmMovimentoProcedimentos.SqlCadastro.ParamByName('IDPROCEDIMENTOS').AsInteger := FrmFiltroProcedimentos.DBLookupComboBoxProcedimentos.KeyValue;
    FrmMovimentoProcedimentos.SqlCadastro.ParamByName('IDMOV_PROCEDIMENTOS').AsInteger := -1;
    ...
    FrmMovimentoProcedimentos.DESCRICAO_PROCEDIMENTOS := FrmFiltroProcedimentos.DBLookupComboBoxProcedimentos.Text;
    FrmMovimentoProcedimentos.ENVOLVE_ADVOGADO := DM.DsProcedimentos.DataSet.fieldbyname('ENVOLVE_ADVOGADO').AsString;
    FrmMovimentoProcedimentos.ENVOLVE_FUNCIONARIO := DM.DsProcedimentos.DataSet.fieldbyname('ENVOLVE_FUNCIONARIO').AsString;
    FrmMovimentoProcedimentos.ENVOLVE_INTERNO := DM.DsProcedimentos.DataSet.fieldbyname('ENVOLVE_INTERNO').AsString;
    FrmMovimentoProcedimentos.ENVOLVE_VISITANTE := DM.DsProcedimentos.DataSet.fieldbyname('ENVOLVE_VISITANTE').AsString;
    FrmMovimentoProcedimentos.HORA_INICIAL := DM.DsProcedimentos.DataSet.fieldbyname('HORA_INICIAL').AsDateTime;
    FrmMovimentoProcedimentos.HORA_FINAL := DM.DsProcedimentos.DataSet.fieldbyname('HORA_FINAL').AsDateTime;
    FrmMovimentoProcedimentos.OBSERVACAO := DM.DsProcedimentos.DataSet.fieldbyname('OBSERVACAO').AsString;
    FrmMovimentoProcedimentos.FUNCAO_FUNCIONARIO := DM.DsProcedimentos.DataSet.fieldbyname('FUNCAO_FUNCIONARIO').AsString;
    FrmMovimentoProcedimentos.Caption := FrmFiltroProcedimentos.DBLookupComboBoxProcedimentos.Text;
    FrmMovimentoProcedimentos.PanelTituloModeloCadastro.Caption := FrmFiltroProcedimentos.DBLookupComboBoxProcedimentos.Text;
...
begin
  ...
  begin
    GLOBAL_PADRAO_SISTEMA := dm.DsPadrao.DataSet.FieldByName('SISTEMA').AsString;
  ...
  if (PERMISSAO_AGENDAATENDIMENTO = '') or (PERMISSAO_AGENDAATENDIMENTO = 'R') then
  begin
  ...
  if ((PERMISSAO_PAVILHAO = '') or (PERMISSAO_PAVILHAO = 'R'))
    and ((PERMISSAO_GALERIA = '') or (PERMISSAO_GALERIA = 'R'))
    and ((PERMISSAO_SOLARIO = '') or (PERMISSAO_SOLARIO = 'R'))
    and ((PERMISSAO_CELA = '') or (PERMISSAO_CELA = 'R'))
    and ((PERMISSAO_CADASTRO = '') or (PERMISSAO_CADASTRO = 'R'))
    and ((PERMISSAO_CONDICAOINTERNO = '') or (PERMISSAO_CONDICAOINTERNO = 'R'))
    and ((PERMISSAO_ADVOGADO = '') or (PERMISSAO_ADVOGADO = 'R'))
    and ((PERMISSAO_FACCAO = '') or (PERMISSAO_FACCAO = 'R'))
    and ((PERMISSAO_CIDADE = '') or (PERMISSAO_CIDADE = 'R'))
    and ((PERMISSAO_RACA = '') or (PERMISSAO_RACA = 'R'))
    and ((PERMISSAO_ESCOLARIDADE = '') or (PERMISSAO_ESCOLARIDADE = 'R'))
    and ((PERMISSAO_NACIONALIDADE = '') or (PERMISSAO_NACIONALIDADE = 'R'))
    and ((PERMISSAO_PROCEDENCIA = '') or (PERMISSAO_PROCEDENCIA = 'R'))
    and ((PERMISSAO_DESTINO = '') or (PERMISSAO_DESTINO = 'R'))
    and ((PERMISSAO_FORNECEDOR = '') or (PERMISSAO_FORNECEDOR = 'R'))
    and ((PERMISSAO_PROFISSAO = '') or (PERMISSAO_PROFISSAO = 'R'))
    and ((PERMISSAO_CTC = '') or (PERMISSAO_CTC = 'R')) then
  begin
  ...
  begin
    ...
    if (PERMISSAO_CONDICAOINTERNO = '') or (PERMISSAO_CONDICAOINTERNO = 'R') then
    begin
  ...
  //************ MOVIMENTAÇÃO ****************//
  if ((PERMISSAO_TRANSFERENCIAINTERNO = '') or (PERMISSAO_TRANSFERENCIAINTERNO = 'R'))
    and ((PERMISSAO_MUDANCACELA = '') or (PERMISSAO_MUDANCACELA = 'R'))
    and ((PERMISSAO_SAIDAO = '') or (PERMISSAO_SAIDAO = 'R'))
    and ((PERMISSAO_CIRCULACAOINTERNO = '') or (PERMISSAO_CIRCULACAOINTERNO = 'R'))
    and ((PERMISSAO_MOVIMENTOSEMIABERTO = '') or (PERMISSAO_MOVIMENTOSEMIABERTO = 'R')) then
  begin
    Movimentao1.Visible := False;
  end
  else
  begin
    if (PERMISSAO_TRANSFERENCIAINTERNO = '') or (PERMISSAO_TRANSFERENCIAINTERNO = 'R') then
    begin
    ...
    if (PERMISSAO_CIRCULACAOINTERNO = '') or (PERMISSAO_CIRCULACAOINTERNO = 'R') then
    begin
      CirculaodeInterno1.Visible := False;
    end;

    if (PERMISSAO_MOVIMENTOSEMIABERTO = '') or (PERMISSAO_MOVIMENTOSEMIABERTO = 'R') then
    begin
      MovimentoSemiAberto1.Visible := False;
    end;

    if not (ContemValor('C', PERMISSAO_TRANSFERENCIAINTERNO) or ContemValor('C', PERMISSAO_MUDANCACELA)) then
    begin
  ...
  //************ SETORES ****************//
  if ((PERMISSAO_DISCIPLINA = '') or (PERMISSAO_DISCIPLINA = 'R'))
    and ((PERMISSAO_FALTADISCIPLINAR = '') or (PERMISSAO_FALTADISCIPLINAR = 'R')) then
  begin
  ...
  begin
    ...
    if (PERMISSAO_FALTADISCIPLINAR = '') or (PERMISSAO_FALTADISCIPLINAR = 'R') then
    begin
  ...
  if ((PERMISSAO_EDUCACAO = '') or (PERMISSAO_EDUCACAO = 'R'))
    and ((PERMISSAO_SERIEFASEESTUDO = '') or (PERMISSAO_SERIEFASEESTUDO = 'R'))
    and ((PERMISSAO_LIVROREVISTA = '') or (PERMISSAO_LIVROREVISTA = 'R')) then
  begin
  ...
  begin
    ...
    if (PERMISSAO_SERIEFASEESTUDO = '') or (PERMISSAO_SERIEFASEESTUDO = 'R') then
    begin
  ...
  if ((PERMISSAO_JURIDICA = '') or (PERMISSAO_JURIDICA = 'R'))
    and ((PERMISSAO_PROCESSOCONDENACAO = '') or (PERMISSAO_PROCESSOCONDENACAO = 'R'))
    and ((PERMISSAO_DETRACAO = '') or (PERMISSAO_DETRACAO = 'R'))
    and ((PERMISSAO_INTERRUPCAO = '') or (PERMISSAO_INTERRUPCAO = 'R'))
    and ((PERMISSAO_REMICAO = '') or (PERMISSAO_REMICAO = 'R'))
    and ((PERMISSAO_ARTIGOPENAL = '') or (PERMISSAO_ARTIGOPENAL = 'R'))
    and ((PERMISSAO_VARAEXECUCAO = '') or (PERMISSAO_VARAEXECUCAO = 'R')) then
  begin
  ...
  if ((PERMISSAO_PEDAGOGIA = '') or (PERMISSAO_PEDAGOGIA = 'R'))
    and ((PERMISSAO_SERVICOSOCIAL = '') or (PERMISSAO_SERVICOSOCIAL = 'R'))
    and ((PERMISSAO_TERAPIAOCUPACIONAL = '') or (PERMISSAO_TERAPIAOCUPACIONAL = 'R')) then
  begin
  ...
  if ((PERMISSAO_CLINICAMEDICA = '') or (PERMISSAO_CLINICAMEDICA = 'R'))
    and ((PERMISSAO_ENFERMAGEM = '') or (PERMISSAO_ENFERMAGEM = 'R'))
    and ((PERMISSAO_FARMACIA = '') or (PERMISSAO_FARMACIA = 'R'))
    and ((PERMISSAO_ODONTOLOGIA = '') or (PERMISSAO_ODONTOLOGIA = 'R'))
    and ((PERMISSAO_PSICOLOGIA = '') or (PERMISSAO_PSICOLOGIA = 'R'))
    and ((PERMISSAO_PSIQUIATRIA = '') or (PERMISSAO_PSIQUIATRIA = 'R'))
    and ((PERMISSAO_REMEDIO = '') or (PERMISSAO_REMEDIO = 'R'))
    and ((PERMISSAO_SAUDE = '') or (PERMISSAO_SAUDE = 'R')) then
  begin
    Sade1.Visible := False;
  ...
  if ((PERMISSAO_TRABALHO = '') or (PERMISSAO_TRABALHO = 'R'))
    and ((PERMISSAO_SETORTRABALHOCAD = '') or (PERMISSAO_SETORTRABALHOCAD = 'R'))
    and ((PERMISSAO_FUNCAOINTERNO = '') or (PERMISSAO_FUNCAOINTERNO = 'R'))
    and ((PERMISSAO_CALCULOTRABALHO = '') or (PERMISSAO_CALCULOTRABALHO = 'R'))
    and ((PERMISSAO_REMICAOTRABALHO = '') or (PERMISSAO_REMICAOTRABALHO = 'R')) then
  begin
  ...
  begin
    ...
    if (PERMISSAO_SETORTRABALHOCAD = '') or (PERMISSAO_SETORTRABALHOCAD = 'R') then
    begin
    ...
    if (PERMISSAO_CALCULOTRABALHO = '') or (PERMISSAO_CALCULOTRABALHO = 'R') then
    begin
      ClculodeTrabalho1.Visible := False;
    end;

    if (PERMISSAO_REMICAOTRABALHO = '') or (PERMISSAO_REMICAOTRABALHO = 'R') then
    begin
  ...
  if ((PERMISSAO_OCORRENCIA = '') or (PERMISSAO_OCORRENCIA = 'R'))
    and ((PERMISSAO_ROUPAS = '') or (PERMISSAO_ROUPAS = 'R')) then
  begin
  ...
  if (PERMISSAO_CONSELHODISCIPLINAR = '') or (PERMISSAO_CONSELHODISCIPLINAR = 'R') then
  begin
  ...
  if (disciplina1.Visible = False)
    and (SetordeEducao1.Visible = False)
    and (Jurdica1.Visible = False)
    and (PSICOSSOCIAL1.Visible = False)
    and (Reabilitao1.Visible = False)
    and (Sade1.Visible = False)
    and (SetorTrabalho1.Visible = False)
    and (Planto1.Visible = False)
    and (ConselhoDi1.Visible = False)
    and (mnuCorrespondencia.Visible = False) then
  begin
  ...
  //************ VISITANTES ****************//
  if (not (PERMISSAO_ENTRADAVISITANTE = 'S'))
    and ((PERMISSAO_GRAUPARENTESCO = '') or (PERMISSAO_GRAUPARENTESCO = 'R'))
    and ((PERMISSAO_VISITANTE = '') or (PERMISSAO_VISITANTE = 'R'))
    and ((PERMISSAO_VISITANTETRAFICO = '') or (PERMISSAO_VISITANTETRAFICO = 'R')) then
  begin
  ...
  begin
    ...
    if (PERMISSAO_VISITANTETRAFICO = '') or (PERMISSAO_VISITANTETRAFICO = 'R') then
    begin
  ...
  and ((PERMISSAO_MONITORAMENTO = '') or (PERMISSAO_MONITORAMENTO = 'R'))
    and ((PERMISSAO_MONITORAMENTOEVENTOS = '') or (PERMISSAO_MONITORAMENTOEVENTOS = 'R')) then
  begin
  ...
  if ((PERMISSAO_FUNCIONARIO = '') or (PERMISSAO_FUNCIONARIO = 'R'))
    and ((PERMISSAO_FUNCAOFUNCIONARIO = '') or (PERMISSAO_FUNCAOFUNCIONARIO = 'R'))
    and ((PERMISSAO_UNIDADEPENAL = '') or (PERMISSAO_UNIDADEPENAL = 'R'))
    and ((PERMISSAO_HORARIOFUNCIONARIO = '') or (PERMISSAO_HORARIOFUNCIONARIO = 'R'))
    and ((PERMISSAO_PADRAOSISTEMA = '') or (PERMISSAO_PADRAOSISTEMA = 'R'))
    and ((PERMISSAO_EQUIPE = '') or (PERMISSAO_EQUIPE = 'R'))
    and ((PERMISSAO_LOCALPOSTOTRABALHO = '') or (PERMISSAO_LOCALPOSTOTRABALHO = 'R'))
  ...
  begin
    ...
    if (PERMISSAO_FUNCAOFUNCIONARIO = '') or (PERMISSAO_FUNCAOFUNCIONARIO = 'R') then
    begin
    ...
    if (PERMISSAO_HORARIOFUNCIONARIO = '') or (PERMISSAO_HORARIOFUNCIONARIO = 'R') then
    begin
    ...
    if (PERMISSAO_AGENTEPOREQUIPE = '') or (PERMISSAO_AGENTEPOREQUIPE = 'R') then
    begin
  ...
  if ((PERMISSAO_AGENDAATENDIMENTO = '') or (PERMISSAO_AGENDAATENDIMENTO = 'R'))
    and ((PERMISSAO_ASSUNTO = '') or (PERMISSAO_ASSUNTO = 'R'))
    and ((PERMISSAO_SITUACAO = '') or (PERMISSAO_SITUACAO = 'R'))
    and ((PERMISSAO_TIPOCONTATO = '') or (PERMISSAO_TIPOCONTATO = 'R')) then
  begin
  ...
  SqlEscalaPlantao.Sql.Text := 'SELECT * FROM ESCALA_PLANTAO WHERE DATA_PLANTAO = ' + Qs(FormatDateTime('dd.mm.yyyy', DateTimePickerEscala.Date));
  ...
  SQLfuncionario.SQL.Text := 'SELECT F.ID_FUNCIONARIO, F.NOME_FUNCIONARIO, F.FOTO, '
    + ' F.DTNASCIMENTO, F.MATRICULA'
    + ' FROM FUNCIONARIO F '
    + ' WHERE F.ID_FUNCIONARIO=' + inttostr(GLOBAL_ID_FUNCIONARIO);
  DsFuncionario.DataSet.Close;
  DsFuncionario.DataSet.Open;

  SqlUltimosLogs.SQL.Text := 'select first 100 l.data_hora,l.script_reversao,l.ip_maquina from log_sistema l '
      ...
      begin
        ...
        begin
          sMensagemInteligencia := sMensagemInteligencia
            ...
            + #13#10 + 'AGUARDANDO PARECER DO SETOR DE INTELIGÊNCIA.' + #13#10 + #13#10
            ;
          next;
        end;
 ...
 // self.Caption := self.Caption + ' - ' + GetBuildInfo(Application.ExeName) + ' (DEV)';
    self.Caption := self.Caption + ' - ' + GetBuildInfo + ' - Data/Hora da versão: ' + FormatDateTime('dd/MM/yyy hh:mm', FileDateToDateTime(FileAge(Application.ExeName)));
...
begin
  ...
  begin
    ...
    if FrmFiltroPostoTrabalho.ShowModal = mrok then
      GLOBAL_IDPOSTO_TRABALHO := FrmFiltroPostoTrabalho.DBLookupComboBox3.KeyValue;
...
begin
  ...
  FrmMotivoEncerrarProcedimento := TFrmMotivoEncerrarProcedimento.Create(Application);
  FrmMotivoEncerrarProcedimento.IDMOV_PROCEDIMENTOS := DsMovProcDetalhado.DataSet.FieldByName('IDMOV_PROCEDIMENTOS').asinteger;
...
begin
  ...
  DM.SqlExecute.SQL.Text := 'select FIRST 1 * from mov_procedimentos where idmov_procedimentos_principal = '
  ...
  begin
    ...
    FrmQualProcedimento.IDMOV_PROCEDIMENTOS_PRINCIPAL := DsMovProcDetalhado.DataSet.FieldByName('IDMOV_PROCEDIMENTOS').AsInteger;
    FrmQualProcedimento.Data_Procedimento1 := Data1;
    FrmQualProcedimento.Data_Procedimento2 := Data2;
    FrmQualProcedimento.Caption := DsMovProcDetalhado.DataSet.FieldByName('descricao').AsString;
...
begin
  ...
  if (Sender.Name = 'SqlProcedimentosDESCRICAO') or (Sender.Name = 'SqlProcedimentosDATA') then
  begin
    ...
    vID := Trim(Copy(vTextoMemo, Pos('ID:', vTextoMemo) + 3, Length(vTextoMemo)));

    if DsMovProcDetalhado.DataSet.Locate('IDMOV_PROCEDIMENTOS', StrToInt(vID), []) then
    begin
      BuscaOcorrencia;
      Exit;
    end;
  ...
  begin
    ...
    if DsMovProcDetalhado.DataSet.Locate('IDMOV_PROCEDIMENTOS', StrToInt(vID), []) then
    begin
      if Application.MessageBox('Arquivar/Encerrar o procedimento.',
        'Confirme', mb_YesNo + Mb_IconQuestion) = idYes then
      begin
        Encerrar1Click(nil);
        ToolButtonAtualizar.OnClick(nil);
  ...
  begin
    ...
    if DsMovProcDetalhado.DataSet.Locate('IDMOV_PROCEDIMENTOS', StrToInt(vID), []) then
    begin
      if Application.MessageBox('Ativar/Reabrir o procedimento.',
        'Confirme', mb_YesNo + Mb_IconQuestion) = idYes then
      begin
        dm.SQLConnect.ExecuteDirect('UPDATE MOV_PROCEDIMENTOS SET ST=''A'', DATA_ENCERRAMENTO=NULL,'
  ...
  begin
    ...
    if Application.MessageBox('Agendar um procedimento de detalhe?',
      'Confirme', mb_YesNo + Mb_IconQuestion) <> idYes then
      Exit;
    ...
    begin
      ...
      DM.DsProcedimentos.DataSet.Filter := 'IDPROCEDIMENTOS_MESTRE = ' + DsMovProcDetalhado.DataSet.fieldbyname('IDPROCEDIMENTOS').AsString;
      ...
      begin
        ...
        begin
          ...
          FrmMovimentoProcedimentos.Editar.Visible := false;
          FrmMovimentoProcedimentos.Excluir.Visible := false;
          FrmMovimentoProcedimentos.IDMOV_PROCEDIMENTOS_PRINCIPAL := StrToInt(vID);
          FrmMovimentoProcedimentos.DESCRICAO_PROCEDIMENTOS := FrmFiltroProcedimentos.DBLookupComboBoxProcedimentos.Text;
          FrmMovimentoProcedimentos.ENVOLVE_ADVOGADO := DM.DsProcedimentos.DataSet.fieldbyname('ENVOLVE_ADVOGADO').AsString;
          FrmMovimentoProcedimentos.ENVOLVE_FUNCIONARIO := DM.DsProcedimentos.DataSet.fieldbyname('ENVOLVE_FUNCIONARIO').AsString;
          FrmMovimentoProcedimentos.ENVOLVE_INTERNO := DM.DsProcedimentos.DataSet.fieldbyname('ENVOLVE_INTERNO').AsString;
      ...
      begin
        ShowMessage('Não tem procedimento de detalhe deste.');
      end;
...
begin
  ...
  begin
    ...
    frxReport3.Variables.AddVariable('SIAP', 'ID_FUNCIONARIO', GLOBAL_ID_FUNCIONARIO);
    ...
    frxReport3.Variables.AddVariable('SIAP', 'GLOBAL_DEPARTAMENTO', Qs(GLOBAL_DEPARTAMENTO));

    frxReport3.Variables.DeleteVariable('GLOBAL_DIRETORIA');
    frxReport3.Variables.AddVariable('SIAP', 'GLOBAL_DIRETORIA', Qs(GLOBAL_DIRETORIA));

    frxReport3.Variables.DeleteVariable('DATA1');
    frxReport3.Variables.AddVariable('SIAP', 'DATA1', Qs(FormatDateTime('dd.mm.yyyy', Data1)));

    frxReport3.Variables.DeleteVariable('DATA2');
    frxReport3.Variables.AddVariable('SIAP', 'DATA2', Qs(FormatDateTime('dd.mm.yyyy', Data2)));

    frxReport3.Variables.DeleteVariable('IDPOSTO_TRABALHO');
    frxReport3.Variables.AddVariable('SIAP', 'IDPOSTO_TRABALHO', GLOBAL_IDPOSTO_TRABALHO);
...
begin
  ...
  begin
    ...
    frxReportOutros.Variables.AddVariable('SIAP', 'ID_FUNCIONARIO', GLOBAL_ID_FUNCIONARIO);
    frxReportOutros.Variables.DeleteVariable('GLOBAL_ORGAO');
    frxReportOutros.Variables.AddVariable('SIAP', 'GLOBAL_ORGAO', QS(GLOBAL_ORGAO));
    frxReportOutros.Variables.DeleteVariable('GLOBAL_DEPARTAMENTO');
    frxReportOutros.Variables.AddVariable('SIAP', 'GLOBAL_DEPARTAMENTO', Qs(GLOBAL_DEPARTAMENTO));
    frxReportOutros.Variables.DeleteVariable('GLOBAL_DIRETORIA');
    frxReportOutros.Variables.AddVariable('SIAP', 'GLOBAL_DIRETORIA', Qs(GLOBAL_DIRETORIA));
    frxReportOutros.Variables.DeleteVariable('IDMOV_PROCEDIMENTOS');
    frxReportOutros.Variables.AddVariable('SIAP', 'IDMOV_PROCEDIMENTOS', DsMovProcDetalhado.DataSet.FieldByName('IDMOV_PROCEDIMENTOS').asinteger);
    frxReportOutros.Variables.DeleteVariable('IDPOSTO_TRABALHO');
    frxReportOutros.Variables.AddVariable('SIAP', 'IDPOSTO_TRABALHO', GLOBAL_IDPOSTO_TRABALHO);
...
procedure TFrmModuloPrincipal.RadioGroupDiaProcedimentoClick(
  Sender: TObject);
begin
  MontaRelProcedimentos;
...
procedure TFrmModuloPrincipal.ToolButtonPostoTrabalhoClick(
  Sender: TObject);
begin
...
procedure TFrmModuloPrincipal.MensagemdoBancodeDados1Click(
  Sender: TObject);
begin
...
begin
  ...
  begin
    MessageDlg('Informe a funcionário.', mtWarning, [mbOK], 0);
    Abort;
  end;
...
begin
  ...
  if Application.MessageBox('Apagar a escala deste dia da equipe?',
    'Confirme', mb_YesNo + Mb_IconQuestion) = idYes then
  begin
    DM.SQLConnect.ExecuteDirect('DELETE FROM ESCALA_PLANTAO WHERE IDEQUIPE =' + inttostr(DBLookupComboBoxEquipe.KeyValue)
      + ' AND DATA_PLANTAO = '
...
procedure TFrmModuloPrincipal.CdsEscalaPlantaoAfterInsert(
  DataSet: TDataSet);
begin
...
begin
  ...
  begin
    ...
    frxReportOutros.Variables.AddVariable('SIAP', 'DATA_PLANTAO', Qs(FormatDateTime('dd.mm.yyyy', DateTimePickerEscala.Date)));
...
begin
  ...
  begin
    ...
    begin
      GLOBAL_PATCH_RELATORIO := GLOBAL_PATCH_SISTEMA + '..\Relatórios';
    end;

    if not DirectoryExists(GLOBAL_PATCH_RELATORIO) then
    begin
      CreateDir(GLOBAL_PATCH_SISTEMA + '..\Relatórios');
    ...
    begin
      IniciaTransMovimento;
      try
        DM.SQLConnect.ExecuteDirect('update conexao set tela_momento = ' + qs(DBLookupComboBoxUP.Text)
          + ' where idconexao=' + IntToStr(GLOBAL_IDCONEXAO));
      except
      end;
      FinalizaTransMovimento;
...
procedure TFrmModuloPrincipal.RecebimentodeTransferncia1Click(
  Sender: TObject);
begin

  FrmRecebimentoTransferencia := TFrmRecebimentoTransferencia.Create(Application);
...
begin
  ...
  //Manual
  Dir := GLOBAL_MEUS_DOCUMENTOS + '\view\' + IntToStr(GLOBAL_ID_FUNCIONARIO) + '\Manual\';
  ...
  FrmSalvarProgressbar := TFrmSalvarProgressbar.Create(Application);
  FrmSalvarProgressbar.CopyFileWithProgressBar2('..\Manual\Manual.pdf', nome_arquivo);
  freeandnil(FrmSalvarProgressbar);
...
begin
  Frmlancamentoremicaotrabalho := TFrmlancamentoremicaotrabalho.Create(Application);
...
procedure TFrmModuloPrincipal.MonitoramentoEletrnico1Click(
  Sender: TObject);
begin
...
begin
  ...
  //Atribuição de cada setor da penitenciária
  Dir := GLOBAL_MEUS_DOCUMENTOS + '\view\' + IntToStr(GLOBAL_ID_FUNCIONARIO) + '\Manual\';
  ...
  FrmSalvarProgressbar := TFrmSalvarProgressbar.Create(Application);
  FrmSalvarProgressbar.CopyFileWithProgressBar2('..\Manual\Atribuicao_Setores.pdf', nome_arquivo);
  freeandnil(FrmSalvarProgressbar);
...
begin
  ...
  FrmCadastroOcorrenciaPlantao := TFrmCadastroOcorrenciaPlantao.Create(Application);
...
begin
  ...
  begin
    ...
    begin
      ShowMessage('Não encontrou o banco de dados.');
    end;
...
begin
  FrmConsultaLocalizacaoPorProntuario := TFrmConsultaLocalizacaoPorProntuario.Create(Application);
...
procedure TFrmModuloPrincipal.TimeraAtualizaRelAgendaDiaTimer(
  Sender: TObject);
//var
//  PastaUsuario: String;
//  SourcePath: String;
...
procedure TFrmModuloPrincipal.TimerAtualizaRelQtdPresosTimer(
  Sender: TObject);
//var
//  PastaUsuario: String;
//  SourcePath: String;

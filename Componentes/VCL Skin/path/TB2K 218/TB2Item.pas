...

{
  ...
  $jrsoftware: tb2k/Source/TB2Item.pas,v 1.278 2006/01/09 19:22:07 jr Exp $
}
...
uses
  MMSYSTEM, TB2Consts, TB2Common, IMM, TB2Acc,Winskindata,Winskindlg,winskinform;
...
begin
  ...
  try
    ...
    //paint TBitem background
    fsd:=skinmanager.maindata;
    if (not ToolbarStyle) and fsd.active then begin
      if IsSelected then i:=4
      else i:=1;
      temp:= TTBPopupWindow(fwindow).bk;
      r3:=r1;
      offsetrect(r3,2,2);
      drawcanvas.copyrect(r2,temp.canvas,r3);

      if assigned(fsd.menuitem) then
        DrawRect2(drawcanvas.handle,r2,fsd.menuitem.map,
         fsd.menuitem.r,i,5,fsd.menuitem.trans,fsd.menuitem.tile)
      else begin
        if IsSelected then drawcanvas.brush.color:= fsd.colors[csHilight]
        else drawcanvas.brush.color:=fsd.colors[csMenuBG];
        drawcanvas.fillrect(r2);
      end;

      if IsSelected then DrawCanvas.Font.Color := fsd.colors[csHilightText]
      else DrawCanvas.Font.Color := fsd.colors[csMenuText];
...
function SkinNCPaint(Wnd: HWND; DC: HDC; AppData: Longint):boolean;
var rc : TRect;
    fsd:Tskindata;
    temp:Tbitmap;
begin
   fsd:= skinmanager.maindata;
   if (fsd=nil) or (not fsd.active) or(fsd.MenuItemBG=nil)
       or (fsd.MenuItemBG.map.empty) then begin
       result := false;
       exit;
   end;
   ...
   TTBPopupWindow(AppData).bk.assign(temp);
   BitBlt(dc,rc.left ,rc.top,rc.right-rc.left,rc.bottom-rc.Top,
                 temp.Canvas.Handle ,0 ,0 ,Srccopy);
   temp.free;

//   DrawRect2(dc,rc,fsd.MenuItemBG.map,fsd.MenuItemBG.r,1,1,
...
begin
  //skin paint
  if SkinNCPaint(wnd,dc,appdata) then exit;

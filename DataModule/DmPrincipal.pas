...
uses
  ...
  IPPeerClient, System.IOUtils, ShellApi, FireDAC.Stan.Intf,
  ...
  FireDAC.Phys.FB, FireDAC.Phys.FBDef, FireDAC.Moni.Base, FireDAC.Moni.FlatFile;
...
type
  TDM = class(TDataModule)
    ...
    SQLmedico: TFDQuery;
    SQLtipodieta: TFDQuery;
    SqlPadrao: TFDQuery;
    DspPadrao: TDataSetProvider;
    CdsPadrao: TClientDataSet;
    DsPadrao: TDataSource;
    Sqltiposaidao: TFDQuery;
...
begin
  ...
  DB_INIFILE := GLOBAL_PATCH_SISTEMA + '..\config\fdconnections.ini';
  ...
  GLOBAL_ACESSA_PROCED := ArqIni.ReadString('SISAP',
    'GLOBAL_ACESSA_PROCED', '');

  GLOBAL_WEBBROWSER := ArqIni.ReadString('SISAP', 'GLOBAL_WEBBROWSER',
    'http://spf.mj.gov.br');
  GLOBAL_SPACECOM := ArqIni.ReadString('SISAP', 'GLOBAL_SPACECOM', '');

  AISSE3_PATH_EXE := ArqIni.ReadString('SISAP', 'AISSE3_PATH_EXE',
    'C:\Program Files\AbsInt\aiSee3\bin\aisee3.exe');
  ...
  Atualizar_Campos_Tabelas_Banco_Dados := ArqIni.ReadString('SISAP',
    'Atualizar_Campos_Tabelas_Banco_Dados', 'S');
 ...
 // SQLConnect.Params.LoadFromFile(DB_INIFILE);
  // SQLConnect.Params.Values['DriverID'] := 'FB';
  ...
  if not(VISUALIZA_OUTRAS_UP = 'S') then
    SqlUP.SQL.Text := 'SELECT * FROM UNIDADE_PENAL WHERE ID_UP = ' +
      inttostr(GLOBAL_ID_UP) + ' ORDER BY NOME_UP '
  else
  ...
  begin
    Sqlservidor.SQL.Text :=
      'select * from funcionario left join perfil_usuario ' +
      ' on (perfil_usuario.id_perfil_usuario = funcionario.id_perfil_usuario) '
      + ' where login=' + Qs(LOGIN_CONECTADO) + ' and SENHA = ' +
      Qs(Senha(GLOBAL_SENHA_USUARIO));
    ...
    begin
      ...
      GLOBAL_ID_FUNCIONARIO := Dsservidor.DataSet.FieldByName('ID_FUNCIONARIO')
        .AsInteger;
      GLOBAL_IDPOSTO_TRABALHO := Dsservidor.DataSet.FieldByName
      ...
      PERMISSAO_CONFERE := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_CONFERE').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_CONFERE_1').AsString,
        'CEDIR');

      PERMISSAO_VISITANTE := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_VISITANTE').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_VISITANTE_1')
        .AsString, 'CEDIR');

      PERMISSAO_TRABALHO := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_SETORTRABALHO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_SETORTRABALHO_1')
        .AsString, 'CEDIR');

      PERMISSAO_CADASTRO := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_CADASTRO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_CADASTRO_1')
        .AsString, 'CEDIR');

      PERMISSAO_EDUCACAO := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_SETOREDUCACAO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_SETOREDUCACAO_1')
        .AsString, 'CEDIR');

      PERMISSAO_PSICOSSOCIAL :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_PSICOSSOCIAL').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_PSICOSSOCIAL_1')
        .AsString, 'CEDIR');

      PERMISSAO_JURIDICA := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_JURIDICA').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_JURIDICA_1')
        .AsString, 'CEDIR');

      PERMISSAO_DISCIPLINA := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_DISCIPLINA').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_DISCIPLINA_1')
        .AsString, 'CEDIR');

      PERMISSAO_INTELIGENCIA :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_INTELIGENCIA').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_INTELIGENCIA_1')
        .AsString, 'CEDIR');

      PERMISSAO_ENFERMAGEM := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_ENFERMAGEM').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_ENFERMAGEM_1')
        .AsString, 'CEDIR');

      PERMISSAO_FARMACIA := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_FARMACIA').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_FARMACIA_1')
        .AsString, 'CEDIR');

      PERMISSAO_CLINICAMEDICA :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_CLINICAMEDICA').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_CLINICAMEDICA_1')
        .AsString, 'CEDIR');

      PERMISSAO_PSICOLOGIA := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_PSICOLOGIA').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_PSICOLOGIA_1')
        .AsString, 'CEDIR');

      PERMISSAO_PSIQUIATRIA := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_PSIQUIATRIA').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_PSIQUIATRIA_1')
        .AsString, 'CEDIR');

      PERMISSAO_SAUDE := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_SAUDE').AsString,
      ...
      PERMISSAO_TERAPIAOCUPACIONAL :=
        ...
        Dsservidor.DataSet.FieldByName('PERMISSAO_TERAPIAOCUPACIONAL_1')
        .AsString, 'CEDIR');

      PERMISSAO_ODONTOLOGIA := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_ODONTOLOGIA').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_ODONTOLOGIA_1')
        .AsString, 'CEDIR');

      PERMISSAO_PEDAGOGIA := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_PEDAGOGIA').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_PEDAGOGIA_1')
        .AsString, 'CEDIR');

      PERMISSAO_SERVICOSOCIAL :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_SERVICOSOCIAL').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_SERVICOSOCIAL_1')
        .AsString, 'CEDIR');

      PERMISSAO_ARMAS := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_ARMAS').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_ARMAS_1').AsString, 'CEDIR');

      PERMISSAO_MONITORAMENTO :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_MONITORAMENTO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_MONITORAMENTO_1')
        .AsString, 'CEDIR');

      PERMISSAO_OCORRENCIA := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_OCORRENCIA').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_OCORRENCIA_1').AsString,
        'CEDIRP');

      PERMISSAO_CONSELHODISCIPLINAR :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_CONSELHODISCIPLINAR').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_CONSELHODISCIPLINAR_1')
        .AsString, 'CEDIR');

      PERMISSAO_TRANSFERENCIAINTERNO :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_TRANSFERENCIAINTERNO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_TRANSFERENCIAINTERNO_1')
        .AsString, 'CEDIR');

      PERMISSAO_MUDANCACELA := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_MUDANCACELA').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_MUDANCACELA_1')
        .AsString, 'CEDIR');

      PERMISSAO_SAIDAO := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_SAIDAO').AsString,
      ...
      PERMISSAO_SAIDAO_CADASTRO :=
        ...
        Dsservidor.DataSet.FieldByName('PERMISSAO_SAIDAO_CADASTRO_1')
        .AsString, 'CEDIR');

      PERMISSAO_CIRCULACAOINTERNO :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_CIRCULACAOINTERNO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_CIRCULACAOINTERNO_1')
        .AsString, 'CEDIR');

      PERMISSAO_MOVIMENTOSEMIABERTO :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_MOVIMENTOSEMIABERTO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_MOVIMENTOSEMIABERTO_1')
        .AsString, 'CEDIR');

      PERMISSAO_FUNCIONARIO := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_FUNCIONARIO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_FUNCIONARIO_1')
        .AsString, 'CEDIR');

      PERMISSAO_FUNCAOFUNCIONARIO :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_FUNCAOFUNCIONARIO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_FUNCAOFUNCIONARIO_1')
        .AsString, 'CEDIR');

      PERMISSAO_UNIDADEPENAL :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_UNIDADEPENAL').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_UNIDADEPENAL_1')
        .AsString, 'CEDIR');

      PERMISSAO_HORARIOFUNCIONARIO :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_HORARIOFUNCIONARIO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_HORARIOFUNCIONARIO_1')
        .AsString, 'CEDIR');

      PERMISSAO_PADRAOSISTEMA :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_PADRAOSISTEMA').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_PADRAOSISTEMA_1')
        .AsString, 'CEDIR');

      PERMISSAO_EQUIPE := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_EQUIPE').AsString,
      ...
      PERMISSAO_LOCALPOSTOTRABALHO :=
        ...
        Dsservidor.DataSet.FieldByName('PERMISSAO_LOCALPOSTOTRABALHO_1')
        .AsString, 'CEDIR');

      PERMISSAO_AGENTEPOREQUIPE :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_AGENTEPOREQUIPE').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_AGENTEPOREQUIPE_1')
        .AsString, 'CEDIR');

      PERMISSAO_REGRAVISITACAO :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_REGRAVISITACAO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_REGRAVISITACAO_1')
        .AsString, 'CEDIR');

      PERMISSAO_PERFILUSUARIO :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_PERFILUSUARIO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_PERFILUSUARIO_1')
        .AsString, 'CEDIR');

      PERMISSAO_ADVOGADO := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_ADVOGADO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_ADVOGADO_1')
        .AsString, 'CEDIR');

      PERMISSAO_VISITANTETRAFICO :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_VISITANTETRAFICO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_VISITANTETRAFICO_1')
        .AsString, 'CEDIR');

      PERMISSAO_FACCAO := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_FACCAO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_FACCAO_1').AsString, 'CEDIR');

      PERMISSAO_CTC := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_CTC').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_CTC_1').AsString, 'CEDIR');

      PERMISSAO_CORRESPONDENCIA :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_CORRESPONDENCIA').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_CORRESPONDENCIA_1')
        .AsString, 'CEDIR');

      PERMISSAO_AGENDAATENDIMENTO :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_AGENDAATENDIMENTO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_AGENDAATENDIMENTO_1')
        .AsString, 'CEDIR');

      PERMISSAO_LIVROREVISTA :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_LIVROREVISTA').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_LIVROREVISTA_1')
        .AsString, 'CEDIR');

      PERMISSAO_PAVILHAO := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_PAVILHAO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_PAVILHAO_1')
        .AsString, 'CEDIR');

      PERMISSAO_GALERIA := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_GALERIA').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_GALERIA_1').AsString,
        'CEDIR');

      PERMISSAO_SOLARIO := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_SOLARIO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_SOLARIO_1').AsString,
        'CEDIR');

      PERMISSAO_CELA := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_CELA').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_CELA_1').AsString, 'CEDIR');

      PERMISSAO_CONDICAOINTERNO :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_CONDICAOINTERNO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_CONDICAOINTERNO_1')
        .AsString, 'CEDIR');

      PERMISSAO_CIDADE := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_CIDADE').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_CIDADE_1').AsString, 'CEDIR');

      PERMISSAO_RACA := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_RACA').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_RACA_1').AsString, 'CEDIR');

      PERMISSAO_ESCOLARIDADE :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_ESCOLARIDADE').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_ESCOLARIDADE_1')
        .AsString, 'CEDIR');

      PERMISSAO_NACIONALIDADE :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_NACIONALIDADE').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_NACIONALIDADE_1')
        .AsString, 'CEDIR');

      PERMISSAO_PROCEDENCIA := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_PROCEDENCIA').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_PROCEDENCIA_1')
        .AsString, 'CEDIR');

      PERMISSAO_DESTINO := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_DESTINO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_DESTINO_1').AsString,
        'CEDIR');

      PERMISSAO_FORNECEDOR := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_FORNECEDOR').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_FORNECEDOR_1')
        .AsString, 'CEDIR');

      PERMISSAO_PROFISSAO := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_PROFISSAO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_PROFISSAO_1')
        .AsString, 'CEDIR');

      PERMISSAO_FALTADISCIPLINAR :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_FALTADISCIPLINAR').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_FALTADISCIPLINAR_1')
        .AsString, 'CEDIR');

      PERMISSAO_SERIEFASEESTUDO :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_SERIEFASEESTUDO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_SERIEFASEESTUDO_1')
        .AsString, 'CEDIR');

      PERMISSAO_PROCESSOCONDENACAO :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_PROCESSOCONDENACAO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_PROCESSOCONDENACAO_1')
        .AsString, 'CEDIR');

      PERMISSAO_DETRACAO := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_DETRACAO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_DETRACAO_1')
        .AsString, 'CEDIR');

      PERMISSAO_INTERRUPCAO := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_INTERRUPCAO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_INTERRUPCAO_1')
        .AsString, 'CEDIR');

      PERMISSAO_REMICAO := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_REMICAO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_REMICAO_1').AsString,
        'CEDIR');

      PERMISSAO_ARTIGOPENAL := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_ARTIGOPENAL').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_ARTIGOPENAL_1')
        .AsString, 'CEDIR');

      PERMISSAO_VARAEXECUCAO :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_VARAEXECUCAO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_VARAEXECUCAO_1')
        .AsString, 'CEDIR');

      PERMISSAO_REMEDIO := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_REMEDIO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_REMEDIO_1').AsString,
        'CEDIR');

      PERMISSAO_FUNCAOINTERNO :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_FUNCAOINTERNO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_FUNCAOINTERNO_1')
        .AsString, 'CEDIR');

      PERMISSAO_CALCULOTRABALHO :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_CALCULOTRABALHO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_CALCULOTRABALHO_1')
        .AsString, 'CEDIR');

      PERMISSAO_REMICAOTRABALHO :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_REMICAOTRABALHO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_REMICAOTRABALHO_1')
        .AsString, 'CEDIR');

      PERMISSAO_ROUPAS := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_ROUPAS').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_ROUPAS_1').AsString, 'CEDIR');

      PERMISSAO_GRAUPARENTESCO :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_GRAUPARENTESCO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_GRAUPARENTESCO_1')
        .AsString, 'CEDIR');

      PERMISSAO_TIPOCONTATO := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_TIPOCONTATO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_TIPOCONTATO_1')
        .AsString, 'CEDIR');

      PERMISSAO_ASSUNTO := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_ASSUNTO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_ASSUNTO_1').AsString,
        'CEDIR');

      PERMISSAO_SITUACAO := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_SITUACAO').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_SITUACAO_1')
        .AsString, 'CEDIR');

      PERMISSAO_SETORTRABALHOCAD :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_SETORTRABALHOCAD').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_SETORTRABALHOCAD_1')
        .AsString, 'CEDIR');

      PERMISSAO_EVENTOS := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_EVENTOS').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_EVENTOS_1').AsString,
        'CEDIR');

      PERMISSAO_SUBEVENTOS := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('PERMISSAO_SUBEVENTOS').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_SUBEVENTOS_1')
        .AsString, 'CEDIR');

      PERMISSAO_MONITORAMENTOEVENTOS :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('PERMISSAO_MONITORAMENTOEVENTOS').AsString,
        Dsservidor.DataSet.FieldByName('PERMISSAO_MONITORAMENTOEVENTOS_1')
        .AsString, 'CEDIR');
      ...
      PERMISSAO_ENTRADAVISITANTE :=
        ...
        Dsservidor.DataSet.FieldByName('PERMISSAO_ENTRADAVISITANTE_1')
        .AsString, 'SN');

      VISUALIZA_OUTRAS_UP := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('VISUALIZA_OUTRAS_UP').AsString,
        Dsservidor.DataSet.FieldByName('VISUALIZA_OUTRAS_UP_1').AsString, 'SN');

      DEFINE_PERFIL_USUARIO := MesclarPermissoes
        (Dsservidor.DataSet.FieldByName('DEFINE_PERFIL_USUARIO').AsString,
        Dsservidor.DataSet.FieldByName('DEFINE_PERFIL_USUARIO_1')
        .AsString, 'SN');

      CONCEDE_PERMISSAO_INDIVIDUAL :=
        MesclarPermissoes(Dsservidor.DataSet.FieldByName
        ('CONCEDE_PERMISSAO_INDIVIDUAL').AsString,
        Dsservidor.DataSet.FieldByName('CONCEDE_PERMISSAO_INDIVIDUAL_1')
        .AsString, 'SN');

      GLOBAL_NOME_FUNCIONARIO_LOGADO := Dsservidor.DataSet.FieldByName
        ('NOME_FUNCIONARIO').AsString;
      NOME_PERFILUSUARIO_LOGADO := Dsservidor.DataSet.FieldByName
        ('PERFIL').AsString;
    ...
    begin
      ...
      begin
        Application.MessageBox
          ('Login e senha não reconhecidos neste banco de dados, lançamentos não serão salvos.',
    ...
    begin
      ...
      try
        ...
        SQLConnect.ExecSQL
          ...
          + ' values (' + inttostr(GLOBAL_IDCONEXAO) + ', ' +
          inttostr(GLOBAL_ID_FUNCIONARIO) + ', current_timestamp, ' +
          Qs('Tela de Login') + ')');
        SQLConnect.ExecSQL('EXECUTE PROCEDURE SET_CONTEXT_CONEXAO(' +
...
begin
  ...
  try
    if DM.SQLConnect.Connected then
    begin
      DM.SQLConnect.Close;
    end;
  finally
    // Aqui você pode adicionar código adicional que precisa ser executado
    // independentemente da conexão ser fechada com sucesso ou não.
  end;
  { while SQLConnect.Connected do
    begin
    try
  ...
  begin
    ...
    try
      DM.SQLConnect.ExecSQL
        ('update conexao set data_hora_saida=current_timestamp, tela_momento = '
        + Qs('Saiu do Sistema.') + ' where idconexao = ' +
        inttostr(GLOBAL_IDCONEXAO));

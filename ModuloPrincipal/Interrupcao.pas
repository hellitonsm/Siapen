...
type
  TFrmInterrupcao = class(TFrmModeloCadastro)
    Label2: TLabel;
...
var
  FrmInterrupcao: TFrmInterrupcao;
...
procedure TFrmInterrupcao.FormCreate(Sender: TObject);
begin
  inherited;
  ...
  if not ContemValor('I', PERMISSAO_INTERRUPCAO)
    and not ContemValor('E', PERMISSAO_INTERRUPCAO)
    and not ContemValor('D', PERMISSAO_INTERRUPCAO) then
  begin
    Novo.Visible := False;
  ...
  if ContemValor('I', PERMISSAO_INTERRUPCAO)
    and not ContemValor('E', PERMISSAO_INTERRUPCAO)
    and not ContemValor('D', PERMISSAO_INTERRUPCAO) then
  begin
    Editar.Visible := False;
  ...
  if ContemValor('I', PERMISSAO_INTERRUPCAO)
    and ContemValor('E', PERMISSAO_INTERRUPCAO)
    and not ContemValor('D', PERMISSAO_INTERRUPCAO) then
  begin
    Excluir.Visible := False;
  end;
  ...
  if not ContemValor('I', PERMISSAO_INTERRUPCAO)
    and ContemValor('E', PERMISSAO_INTERRUPCAO)
    and ContemValor('D', PERMISSAO_INTERRUPCAO) then
  begin
    Novo.Visible := False;
  end;
...
procedure TFrmInterrupcao.CdsCadastroAfterInsert(DataSet: TDataSet);
begin
  inherited;
  DsCadastro.DataSet.fieldbyname('IDINTERRUPCAO').AsInteger := 0;
...
procedure TFrmInterrupcao.NovoClick(Sender: TObject);
begin
...
procedure TFrmInterrupcao.CalculaPeriodo();
var
...
procedure TFrmInterrupcao.DsCadastroDataChange(Sender: TObject;
  Field: TField);
begin
  inherited;
...
procedure TFrmInterrupcao.SalvarClick(Sender: TObject);
begin
  CalculaPeriodo();
...
procedure TFrmInterrupcao.BtnIncluirDocDigitalizadoClick(Sender: TObject);
var
...
procedure TFrmInterrupcao.BtnVisuzalizarDocDigitalizadorClick(
  Sender: TObject);
var
  nome_arquivo: string;
  Dir: string;
...
procedure TFrmInterrupcao.Button1Click(Sender: TObject);
begin
  inherited;
  CalculaPeriodo();
